...
- 日期：2026-06-22
- 最后复核：2026-07-21
- 状态：目标设计；基础 personId 注册已落地，完整人员域与权限系统未实现
...
- Android 侧已经具备摄像头权限状态机、安全模式和独立的 `FeatureTemplateEncryptedStore` 工具；当前未找到该存储工具接入人脸注册按钮或完整人员工作流的调用证据。
...
| 能力 | 状态 | 本地证据与限制 |
|---|---|---|
| Windows/Web 基础注册 | 已落地 | `POST /api/v1/actions/enroll` 接受 `personId`，Web 预览页可发起请求；接口返回表示请求已受理，不表示采样已完成 |
| Windows 清库 | 已落地 | `POST /api/v1/actions/db/clear` 触发异步清库命令 |
| Windows 人脸库 | 已落地（原型） | `FaceDatabase::PersonEntry` 仅含 `id`、均值向量和样本数 |
| Android 加密模板存储工具 | 已落地（独立能力） | 使用 Android Keystore/AES-GCM；尚未证明已接入注册 UI 或双端同步 |
| `PersonProfile` / `BiometricProfile` 领域模型 | 未实现 | 本文第 5 节为目标模型 |
| 人员冻结、删除单人、重复人检测、质量门禁 | 未实现 | 当前公开动作仅基础注册与整库清空 |
| RBAC + ABAC、操作员会话、后端鉴权 | 未实现 | 当前本地 REST API 不能视为已具备人员权限治理 |
| 审计检索、审批、离线采集同步、多站点 | 未实现 | 本文相关章节均为后续设计，不是现有能力 |

除“已落地”行外，本文后续服务、对象、接口和流程均为目标设计。实施时必须重新核对接口契约与数据迁移，不得直接把设计字段解释为现有代码。
...
为避免 `personUuid` 进入领域层后，底层识别引擎仍然直接以 `personId` 充当唯一键，系统必须引入单独的识别适配键策略。

#### 键分层定义

- `personUuid`：人员域内部不可变主键，是人员、模板、审计、合并和删除的统一关联键。
- `personId`：业务可读标识，允许在受控条件下修改。
- `recognizerSubjectId`：识别引擎内部使用的主题键，用于底层模板索引和识别匹配。

#### 第一阶段策略

- 第一阶段允许 `recognizerSubjectId` 继续沿用现有 `personId` 形式，以降低对现有 `IRecognizer`、`FramePipeline`、`BioAuth` 的改动量。
- 但业务层禁止直接把 `personId` 当作内部主键使用，必须通过 `RecognitionAdapter` 从 `personUuid` 映射到 `recognizerSubjectId`。
- 识别结果返回后，统一由适配层翻译成：
  - `personUuid`
  - `personId`
  - `displayName`
...
- 当识别链路具备升级条件后，可将 `recognizerSubjectId` 从 `personId` 迁移到独立内部键，进一步解除对业务编号的耦合。
- 该演进不影响上层域模型，因为上层始终只依赖 `personUuid`。
...
为支持 `personUuid` 引入后的重复人治理，系统需要定义明确的人员合并规则。

#### 合并触发条件

- `DuplicateSuspected` 状态下，人工判定两个档案属于同一自然人。
- 历史导入数据清洗时，发现同一人存在多个旧 `personId`。
- 运营维护中确认同一人员因改名、重注册或误建档产生重复记录。

#### 合并处理规则

- 选择一个 `personUuid` 作为主档保留，其余档案转为被合并档。
- 主档保留最新有效状态、模板主记录和当前展示信息。
- 被合并档的历史 `personId` 不直接丢弃，而是进入别名或历史标识映射表，确保旧审计记录和旧识别结果仍可追溯。
- 审计记录不做物理合并重写，只通过 `personUuid` 映射到主档展示。
- 若被合并档存在质量更高的模板，可在人工确认后并入主档模板集。

#### 合并约束

- 合并必须由具备相应权限的管理员执行。
- 合并前必须生成预览信息，列出两边档案、模板数、最近活动时间和风险提示。
- 合并后必须写强审计，记录主档、被合并档、操作者和原因。

### 6.7 误注册保护机制

默认允许自动入库后，必须同步建立误注册保护机制，避免因为现场质量波动导致错误数据永久进入主库。

#### 保护策略

- 自动入库后进入短暂保护窗口，在保护窗口内允许管理员执行快速撤销。
- 保护窗口内的模板记录标记为“新入库未确认”，高风险场景下可限制其立即参与关键授权判断。
- 命中以下任一条件时，禁止自动入库，必须转人工审核：
  - 重复人高相似命中
  - 离线回传且版本/质量存在异常
  - 采集过程存在多人脸抖动或质量边缘值
  - 特殊场景策略明确要求人工复核
...
- 撤销操作应恢复到 `PendingEnrollment` 或 `Draft`，而不是直接删除所有痕迹。
- 撤销后保留误注册审计，供后续质量调参和流程复盘。
...
为避免首个管理员丢失、口令遗忘或策略异常导致系统无法维护，系统需要提供受控的应急恢复机制。

第一阶段提供两类受控恢复路径：

1. 离线恢复包
- 由 Windows 主控端在系统健康状态下预先生成。
- 恢复包只用于重建管理员访问或恢复本地鉴权控制，不用于导出业务敏感数据。
- 恢复包使用单独标识和有效期，并要求二次确认。

2. 本机受控重置流程
- 仅允许在 Windows 本机执行。
- 需要满足显式本机操作、恢复口令或恢复介质校验、写强审计三项条件。
- 受控重置只恢复管理访问能力，不清空人员库和审计库。

应急恢复触发后，系统必须：

- 生成高优先级审计记录。
- 强制提示管理员尽快轮换本地口令和会话。
- 使旧会话失效，避免恢复期间旧权限继续生效。

### 7.7 Android 通信模式

Android 与 Windows 主控端之间允许同时存在两种通信模式，以兼顾当前安全边界和后续场景扩展。

#### 模式 A：本地离线采集包

- Android 允许本地完成采集、质检和缓存。
- 采集结果以离线采集包形式保存，后续通过受控导入方式进入 Windows 主控端。
- 该模式不依赖 Android 与 Windows 的实时联网，是第一阶段必须支持的基础模式。

#### 模式 B：受控局域网模式

- 在明确启用后，Windows 主控端可开启受控局域网通信能力，允许 Android 通过局域网访问专用桥接通道。
- 该通道不等同于现有仅监听 `127.0.0.1` 的 Web SPA 本地接口，而是独立的设备桥接入口。
- 受控局域网模式必须满足以下约束：
  - 默认关闭，需管理员显式开启
  - 使用独立端口和独立审计事件类型
  - 仅允许白名单设备或配对成功设备接入
  - 支持设备信任撤销
  - 关闭后 Android 自动退回离线采集包模式

#### 实施边界

- 第一阶段不要求把现有全部 Web SPA 接口直接暴露到局域网。
- Android 在线登录、权限快照拉取、在线回传应优先走专用桥接能力，而不是直接复用浏览器侧接口。
...
双端模板是否可直接互通，会受到模型类型、模板结构、平台实现和版本差异影响。该问题不在本轮设计中直接定型，单独作为研究项管理。

#### 当前结论

- 本轮设计不强行假设 Android 与 Windows 生成的模板天然可直接互认。
- 第一阶段允许 Android 回传“离线采集包”而非强制回传最终模板，以便由 Windows 主控端按统一规则生成或复算模板。
- 若未来确认某一模型链路支持真正的双端模板直通，再在专项研究完成后升级。

#### 研究输出要求

- 明确标准模板格式及其版本号。
- 明确哪些模型组合支持模板直通，哪些必须回传原始样本。
- 明确重复人检测以哪类模板为主。
- 明确模型升级时的重算策略。

### 8.8 密钥体系研究项

权限快照签名、离线恢复包、局域网桥接鉴权、模板加密涉及不同用途的密钥。为避免在本轮设计中混用现有 DPAPI/AndroidKeyStore 能力，密钥体系单独作为研究项。

<a id="当前结论-2"></a>
#### 当前结论（2）

- 本轮设计只定义“需要签名、验签、设备绑定和加密保护”的能力边界。
- 具体密钥分层、密钥轮换、跨端验签材料分发方式留待专项研究确定。
- 在专项方案确定前，不允许在实现中临时硬编码跨端密钥或复用不适合的配置加密密钥。

<a id="研究输出要求-2"></a>
#### 研究输出要求（2）

- 定义模板加密密钥、快照签名密钥、恢复包密钥、局域网桥接认证材料的职责边界。
- 定义 Windows 主控端与 Android 端的验签材料分发方式。
- 定义失效、轮换和吊销策略。
...
- 在人员域与底层识别引擎之间增加 `RecognitionAdapter` 或等价适配层。
- 该适配层负责：
  - `personUuid -> recognizerSubjectId` 映射
  - 识别结果翻译为 `personUuid + personId + displayName`
  - 兼容旧 `FaceDatabase` 的运行时索引生成
- 业务层禁止直接读取底层 `personIds()` 结果后自行拼装人员信息，避免再次形成键混乱。
...
- 启动向导与首个管理员初始化流程
- 识别适配键策略与 `RecognitionAdapter` 边界
- Android 离线采集包模式定义
...
### 风险 8：局域网桥接能力被误当成普通 Web 接口开放

问题：

- 若直接把现有本地 Web 接口暴露到局域网，会破坏当前仅监听 `127.0.0.1` 的安全边界。

应对：

- 局域网通信必须走独立桥接通道。
- 桥接通道默认关闭，仅允许白名单或配对设备接入。

### 风险 9：主键、业务标识、识别引擎键再次混用

问题：

- 若实现阶段跳过适配层，继续在不同层直接混用 `personUuid`、`personId` 和底层识别键，后续会再次出现映射混乱。

应对：

- 强制引入 `RecognitionAdapter`。
- 领域层只认 `personUuid`，底层识别键只通过适配层暴露。

### 风险 10：模板可移植性和密钥体系被“边做边猜”

问题：

- 若在没有专项研究结论前直接编码，会导致双端模板不兼容、签名验签方案不可维护或后续无法轮换。

应对：

- 将模板可移植性与密钥体系明确列为专项研究项。
- 在专项结论完成前，不允许以临时实现替代正式契约。

## 13. 结论

本设计采用“本地域模型 + 可扩展策略层”的方案，以 Windows 本地服务为主控端、Android 为受限现场端，围绕人员全生命周期、RBAC + ABAC
混合权限、模板加密和审计留痕构建一套轻量但具备生产实践能力的双端系统。该方案能够在不引入企业级重型平台的前提下，补齐当前项目从“识别原型”走向“可落地人员系统”的关键缺口，并为后续特殊场景和多站点扩展预留结构化边界。
