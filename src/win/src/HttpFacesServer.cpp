...
void HttpFacesServer::handleClient(std::uintptr_t sock) {
    ...
    if (req.path == "/api/v1/preview.mjpeg" && req.method == "GET") {
        ...
        while (running_) {
            ...
            // ⚡ Bolt: Replaced std::ostringstream with std::string concatenation to avoid virtual
            // calls and locale overhead per frame in this high-frequency MJPEG stream loop.
            std::string partHead;
            partHead.reserve(64 + boundary.size());
            partHead += "--";
            partHead += boundary;
