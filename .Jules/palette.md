...
## 2025-02-23 - Standalone Input Keyboard Accessibility
**Learning:** For standalone `<Input>` fields associated with an action button (e.g., registration or search), users expect to hit 'Enter' to submit. If the input and button aren't wrapped in a `<Form>` with a submit handler, keyboard users cannot trigger the action.
**Action:** Always bind the action handler to the input's `onPressEnter` event. Ensure the handler manually validates the input (e.g., `!input.trim()`) to maintain functional parity, as the Enter key bypasses the button's `disabled` state.
