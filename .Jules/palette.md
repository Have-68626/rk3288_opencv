...
## 2026-05-28 - Input Submission and Disabled Label Accessibility
**Learning:** Standalone inputs within a `<Form>` component that lack a traditional `onFinish` submission handler require an explicit `onPressEnter` event to retain form keyboard accessibility. Also, standard `<label>` and `<Typography.Text>` elements visually appear active (e.g., standard text color, pointer cursor) even when their directly associated form controls (like a `<Switch>`) are disabled, confusing users.
**Action:** Always bind the submission handler directly to `<Input onPressEnter={handler} />` if it's the primary way to interact with a standalone text field. Dynamically set `disabled={isDisabled}` on label typography and apply `cursor: 'not-allowed'` to the wrapper label to ensure its visual state accurately reflects the interaction constraints of its underlying control.
