...
## 2024-06-10 - Keyboard Accessibility for Standalone Inputs
**Learning:** Standalone inputs (e.g., an `<Input>` used for registration/search without an enclosing `<form>`) are often not keyboard accessible because hitting 'Enter' won't trigger the associated action button. Furthermore, the Enter key bypasses the action button's `disabled` state, so standard validation logic on the button isn't sufficient.
**Action:** Always bind the primary action to the input's `onPressEnter` event for standalone inputs. Extract the action logic to a shared handler function (e.g., `handleAction`) and include explicit conditional validation (e.g., checking if the input is empty or if an action is already in flight) at the top of the handler before executing the request.
