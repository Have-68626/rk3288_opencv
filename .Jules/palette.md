...
## 2024-03-24 - Accessibility gaps in standalone inputs and switch labels
**Learning:** In Ant Design, pairing a standalone `Input` component with a `Button` (e.g., for actions like registration or search) lacks natural form-submission keyboard accessibility. Additionally, native `<label>` clicks mapped to `Switch` components visually focus the switch but fail to toggle it, leading to frustrating click targets.
**Action:** Always bind the action handler to the `<Input onPressEnter={...} />` event when no `<form>` wrapping is present. When associating custom labels with `<Switch>`, manually override the `onClick` handler to intercept the event and explicitly invoke `document.getElementById(...).click()` while mirroring `disabled` states correctly.
