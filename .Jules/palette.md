...
## 2025-02-23 - Keyboard Accessibility for Standalone Input-Button Pairs
**Learning:** When a standalone `<Input>` field is paired visually with an action `<Button>` (like a registration or search input, outside of a standard form submission context), keyboard users are blocked from triggering the action using the "Enter" key if the action is only bound to the button's `onClick`. This creates an inaccessible flow.
**Action:** Always extract the action logic into a reusable handler that validates the input (e.g., checking `!input.trim()`). Bind this handler to both the button's `onClick` and the input's `onPressEnter` event to ensure complete keyboard accessibility and parity with mouse flows.
