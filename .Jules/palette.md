...
## 2026-06-05 - Ant Design Switch Label Interaction
**Learning:** Native `<label htmlFor="...">` clicks on Ant Design `Switch` components only focus the control (which renders as a `<button>`), without toggling the state.
**Action:** When binding `<label>` elements to Ant Design Switches, always explicitly handle the `onClick` event to call `e.preventDefault()` and manually trigger the switch's toggle handler function, while also applying visual disabled states (e.g., `cursor: not-allowed` and text gray-out) to match the switch.
