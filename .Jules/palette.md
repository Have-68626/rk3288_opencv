...
## 2025-06-06 - Accessible Forms Require Enter Key Support
**Learning:** For standalone `<Input>` fields associated with an action button (like registration or search), screen reader and keyboard-only users expect pressing 'Enter' to submit the value. Relying only on a visual action button degrades accessibility because users must tab away from the input to submit it.
**Action:** Always bind the action handler to the input's `onPressEnter` event. Because the Enter key bypasses the `<Button>`'s visual `disabled` state, ensure the shared handler manually validates the input (e.g., `!input.trim()`) to maintain functional parity.
