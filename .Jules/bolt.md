...
## 2026-07-20 - Avoid std::istringstream overhead in HTTP header parsing
**Learning:** `std::istringstream` exhibits significant overhead when used for simple string extraction, such as parsing HTTP request lines or headers in `readRequest`, due to dynamic allocations, virtual function calls, and locale handling. In networking pathways like `HttpFacesServer.cpp`, this results in unnecessary per-request CPU and memory overhead, limiting peak request throughput.
**Action:** Replace `std::istringstream` with zero-allocation string parsing techniques, such as using `find_first_of`, `substr`, and manually iterating over string lines (`getLine` lambda) with offsets. This retains readability while eliminating streaming overhead in hot network paths.
