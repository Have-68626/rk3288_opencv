...
## 2026-07-06 - Avoid redundant cv::Mat::clone before ResultPublisher::publish
**Learning:** In the rendering pipeline (`Engine::processFrame`), calling `cv::Mat::clone()` before passing a frame to `ResultPublisher::publish()` is redundant because the publisher already calls `cv::Mat::copyTo()` internally to safely store the frame. This upstream clone introduces a costly deep copy and unnecessary heap allocations per frame on the hot path.
**Action:** Avoid calling `cv::Mat::clone()` on frame buffers right before passing them to sink endpoints that already perform their own layout isolation or duplication (like `ResultPublisher::publish()`). Pass the matrix by value/reference instead (shallow copy). NOTE: `clone()` must still be used if the frame is modified (e.g., via `cv::putText`) prior to publishing to avoid corrupting the source buffer.
