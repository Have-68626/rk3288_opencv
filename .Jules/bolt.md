...
## 2024-05-24 - Replaced `std::ostringstream` with `snprintf` and `std::string`
**Learning:** `std::ostringstream` has significant virtual dispatch and memory allocation overhead. In hot paths like file hashing or json formatting, it creates performance bottlenecks.
**Action:** Replace `std::ostringstream` with pre-allocated `std::string` concatenation and `snprintf` for primitive formatting to minimize latency and memory overhead. Use `%g` for floats to retain the same precision formatting as `ostringstream`.
