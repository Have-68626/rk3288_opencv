...
## 2026-06-02 - Avoid std::ostringstream overhead when reading full files
**Learning:** Using `std::ostringstream` with `rdbuf()` to read an entire file dynamically allocates memory and causes an additional full string copy during `.str()`. For static assets like `index.html` fetched rapidly, this overhead is measurable.
**Action:** Always pre-size the string buffer based on `tellg()` and read directly into it using `ifs.read(out.data(), size)`.
