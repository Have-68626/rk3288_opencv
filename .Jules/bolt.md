...
## 2026-05-25 - Avoid redundant clone() when creating overlay for MJPEG streaming
**Learning:** In pipelines serving MJPEG streams via HTTP (`src/win/src/HttpFacesServer.cpp`), creating an overlay (like drawing face bounding boxes) typically operates on a thread-local frame snapshot (e.g., `RenderState::bgr`). If this snapshot is already a deep copy guaranteed to be isolated from the main inference pipeline, calling `.clone()` again creates an expensive, redundant memory allocation per frame in a hot path.
**Action:** Replace `cv::Mat img = rs.bgr.clone();` with a shallow copy `cv::Mat img = rs.bgr;` if it's verified that `rs.bgr` is an isolated deep copy. This avoids allocating a new memory buffer per frame while still safely incrementing the reference counter.
