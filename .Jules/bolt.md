...
## 2026-06-24 - Avoid redundant clone() when creating overlay image
**Learning:** In `HttpFacesServer.cpp`, `buildJpegWithOverlay` creates a redundant deep copy of `rs.bgr` using `clone()` before drawing bounding boxes. However, `rs.bgr` is already an isolated thread-local deep copy populated by `FramePipeline::tryGetRenderState()`. Calling `clone()` introduces unnecessary memory allocation overhead.
**Action:** Use a reference (`cv::Mat& img = rs.bgr`) instead of `clone()` when modifying a matrix that is explicitly guaranteed by the pipeline to be an isolated, disposable copy.
