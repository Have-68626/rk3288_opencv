...
## 2024-07-12 - Optimize Engine::renderResults frame allocation
**Learning:** In the rendering pipeline `Engine::renderResults()`, `cv::Mat::clone()` was used to deep-copy the `processedFrame_` into the `FrameOutcome::renderFrame`. However, the downstream `ResultPublisher::publish()` natively uses `copyTo()` under a mutex to safely duplicate the frame data into its own buffer, making the upstream `clone()` completely redundant. The deep copy inside `renderResults` causes unnecessary heap allocation and memory bandwidth overhead per frame.
**Action:** Assign the `processedFrame_` directly to `outcome.renderFrame` (a shallow copy) to eliminate the per-frame allocation while still correctly propagating the frame pointer and keeping OpenCV reference counts intact. Preserve `clone()` if upstream modifications (like `putText`) are needed.
