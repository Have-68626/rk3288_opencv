...
## 2025-02-14 - Fix unmasked sensitive data in diagnostic logs
**Vulnerability:** Diagnostic logs at DEBUG/VERBOSE levels bypassed sensitive data masking, allowing credentials, tokens, and PII to be written in plaintext to disk, which risks exposing high-value information if the device is compromised or logs are accessed without authorization.
**Learning:** Masking shouldn't be skipped for lower log levels under the assumption they're "safe" or "development-only" because production environments may temporarily enable them for troubleshooting, or logs could inadvertently capture secrets.
**Prevention:** Always apply sensitive data masking universally across all log levels before writing to persistent storage. To mitigate potential CPU regressions on resource-constrained SoCs due to the overhead of regex-based masking, employ an allocation-free pre-check (e.g., manually looping over characters to detect digits, '@', ':', or '=') to fast-path lines that definitely do not contain sensitive data.
