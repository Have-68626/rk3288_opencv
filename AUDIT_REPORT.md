...
4. **P1 — 6 个 HTTP API 端点返回 404**：`/api/v1/actions/crypto/rotate`、`/api/v1/actions/privacy/open`、`/api/v1/preview.jpg`、`/api/v1/acceleration` 被前端调用或文档化但服务端未实现（其中 4 个严重、2 个文档级）。其中两个端点直接导致前端功能损坏。
...
- **Windows 本地服务**（`win_local_service`）— 自有 Winsock `HttpFacesServer` 提供 REST API + Web SPA 后端，同时提供 D3D 渲染和摄像头管理
- **CLI 工具**（`rk3288_cli`、`inference_bench_cli`、`opencv_verify_cli`）— 直接链接核心引擎用于调试和基准测试

Web 前端是独立 React 应用，编译为静态文件，由 Windows 本地服务中的自有 Winsock `HttpFacesServer` 提供服务。
...
| # | 类别 | 文件 | 行 | 说明 | 影响 |
|:-:|:-----|:----|:--:|:------|:------|
| H1 | 证书验证 | civetweb.c（第三方归档快照） | 18341 | 历史扫描发现 `SSL_VERIFY_NONE`；CivetWeb 未编入目标 | 非当前运行风险；未来接入 HTTPS 客户端前需修复并验证 |
...
| M2 | 不安全函数 | original_civetweb.c（第三方归档快照） | 多处 | 历史扫描记录 200+ 处 sprintf/strcpy/strcat；未编入目标，非当前运行风险 |
| M3 | 命令执行 | original_civetweb.c（第三方归档快照） | 12581 | 历史扫描记录 `popen(cmd, "r")` CGI 路径；未来接入前需重审 |
| M4 | 命令执行 | original_civetweb.c（第三方归档快照） | 5971+ | 历史扫描记录 `execle()` CGI 路径；未来接入前需重审 |
| M5 | 弱加密 | original_civetweb.c（第三方归档快照） | 1725,8530 | 历史扫描记录 HTTP Digest MD5 与证书指纹 SHA1；未来接入前需重审 |
...
**安全评估总结**：自研 C++ 代码安全质量良好 — 无原始指针数组问题，使用现代 C++ 模式（vector/string/智能指针/RAII）。Android 端实现了 Android KeyStore + AES-256-GCM 加密、日志脱敏、禁止备份等良好安全实践。CivetWeb 发现仅适用于未编入目标的第三方归档快照；**当前运行路径的主要风险为 FFmpeg 命令构造**。
...
| 2 | civetweb.h（第三方归档快照） | 1833 | 452 | — | C |
...
| scripts | 2,899 | **0%** | 🔴 零注释 |
| web（前端） | 2,343 | **1%** | 🔴 极低 |
| third_party（CivetWeb 归档快照） | 1,833 | **26%** | ✅ 第三方自带注释；未编入目标 |
...
| P1.8 | **CivetWeb SSL 验证修复**（历史项，当前不适用；未来接入前处理 `SSL_VERIFY_NONE`） | 第三方归档 | 不适用 | — |
...
| CivetWeb SSL MITM（归档快照） | 高 | 低 | 不适用 | — | 未来接入前使用 SSL_VERIFY_PEER 并补测试 |
| CivetWeb 缓冲区溢出（归档快照） | 高 | 低 | 不适用 | — | 未来接入前重新评估并选择受支持版本 |
| 单点贡献者风险 | 中 | 高 | 🟡 中 | — | 增加维护者 |
| 分支膨胀 | 低 | 高 | 🟢 低 | 低 | 清理远程分支 |
| 注释不足 | 低 | 100% | 🟢 低 | 低 | 逐步补充 |
...
| 3 | 安全热点枚举 | `.claude/audit/p1.3-security-scan.md` | S1 | 21 项历史发现（高危 2）；CivetWeb 结论仅适用于未编入目标的归档快照 |
